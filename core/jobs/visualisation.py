...
raw_df = spark.read.csv('input/visa_number_in_japan.csv', header=True, inferSchema=True)
...
cleaned_df = raw_df.toDF(*new_column_names)

# drop all null columns
cleaned_df = cleaned_df.dropna(how='all')

selected_df = cleaned_df.select('year', 'country', 'number_of_issued_numerical')
...
country_df = selected_df.withColumn('country', correct_country_name_udf(selected_df['country']))
...
replaced_country_df = country_df.replace(country_corrections, subset='country')

continent_udf = udf(get_continent_name, StringType())
replaced_country_df = replaced_country_df.withColumn('continent', continent_udf(replaced_country_df['country']))

# replaced_country_df.show()

replaced_country_df.createGlobalTempView('japan_visa')
...
top_10_country_df = top_10_country_df.toPandas()

fig = px.bar(top_10_country_df, x='country', y='visa_issued', color='country')
...
replaced_country_df.write.csv("output/visa_number_in_japan_cleaned.csv", header=True, mode='overwrite')
